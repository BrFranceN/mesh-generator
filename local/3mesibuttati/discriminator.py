"""
Discriminator that takes 1024 x 1024 x 3 image and predicts the probability of real
...
class Discriminator(nn.Module):
    def __init__(self, config_file):
        super(Discriminator, self).__init__(ndf=64, nc=3)
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(ndf * 64, ndf * 64, 4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(ndf * 64),
            nn.LeakyReLU(0.2, inplace=True),
            ...
            nn.Conv2d(ndf * 64, 1, 2, stride=2, padding=0, bias=False),
            nn.Sigmoid()
        )
